    ...
    <form action="" method="post" class="wc_trendyol_settings_frm">
		...
		<?php
			...
			foreach($settings_inputs as $input_slug => $input){
				...
				$input_name   = $input['name'] ?? $input_slug;
				$input_type   = $input['input_type'] ?? 'checkbox';

				$meta_value = $trendyol_metas->get_wc_trendyol_meta_settings($input_name);
				if($meta_value == $value){
					$checked = true;
				}
				else{
                ...
                <div class="wc_trendyol_form_group_inline">
                    <input name="<?=$input_name?>" type="<?=$input_type?>" class="wc_trendyol_checkbox" id="<?=$input_slug?>" value="<?=$value?>" <?=$checked ? 'checked' : ''?> <?=$disabled ? 'disabled' : ''?>/>
...
<div class="wc_trendyol_faq_list wc_trendyol_col_6" style="position:sticky; top:130px;">
    ...
    <div class="wc_trendyol_content_collapse">
        ...
        <div class="wc_trendyol_collapse_content">
            <a href="https://www.youtube.com/watch?v=aeibEz1hQ28" target="_blank">
                <img src="<?=WC_TRENDYOL_DIR_URL?>/admin/assets/img/trendyol-woocommerce-eklentisi-kurulumu.png" width="100%">
            </a>
        </div>
    </div>
