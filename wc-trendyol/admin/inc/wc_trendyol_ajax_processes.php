	...
	class wc_trendyol_ajax_processes{
		...
		public function wc_trendyol_save_settings(){
			...
			try{
				...
				if((isset($products->body->exception) and $products->body->exception == 'TrendyolAuthorizationException') or (isset($products->status) and $products->status == '404')){
					...
					foreach($wc_trendyol_inputs as $input_slug => $wc_trendyol_input){
						$input_name   = $wc_trendyol_input['name'] ?? $input_slug;
						$trendyol_metas->update_wc_trendyol_meta_settings($input_name, $post[$input_name] ?? null);
					}
		...
		public function wc_trendyol_search_brand(){
			...
			$return = null;
			if(isset($trendyol_brands->body)){
				foreach($trendyol_brands->body as $brand){
		...
		public function wc_trendyol_product_matching_table(){
			...
			$html                = '';
			$page_line_count     = 20;
			$products            = $trendyol_admin->get_wc_products(($wc_cat_id ?? 0), ($paged - 1), $page_line_count);
			$total_product_count = $trendyol_admin->get_wc_products_total(($wc_cat_id ?? 0));
			if($products != null){

				$html_body = '';

				foreach($products as $r_id => $product){
		...
		public function wc_trendyol_categories_matching_table(){
			...
			if($categories != null){
				...
				foreach($categories as $category){
					...
					$convert_option              = $trendyol_admin->trendyol_categories_array_to_select_option($trendyol_categories->body->categories ?? [], 0, ($get_wc_trendyol_category_id ?? 0));
		...
		public function wc_trendyol_change_this_wc_cat_website_product_price(){
			...
			if(!empty($wc_cat_id)){
				...
				if(strlen($value_input) > 0 and $value_input > 0){
					...
					$get_this_cat_products = $trendyol_admin->get_wc_products($wc_cat_id);
					foreach($get_this_cat_products as $wc_product){
		...
		public function wc_trendyol_change_all_website_product_price(){
			...
			if(strlen($value_input) > 0 and $value_input > 0){
				...
				$get_this_cat_products = $trendyol_admin->get_wc_products(null, 1, 100000);
		...
		public function wc_trendyol_change_this_wc_cat_trendyol_product_price(){
			...
			if(!empty($wc_cat_id)){
				...
				if(strlen($value_input) > 0 and $value_input > 0){
					...
					$get_this_cat_products = $trendyol_admin->get_wc_products($wc_cat_id, 1, 100000);
					foreach($get_this_cat_products as $get_wc_product){

						$old_regular_price = $trendyol_metas->get_meta_trendyol_sale_price($get_wc_product->id); //İNDİRİMSİZ FİYAT
						$old_sale_price    = $trendyol_metas->get_meta_trendyol_discount_price($get_wc_product->id); //İNDİRİMLİ FİYAT
