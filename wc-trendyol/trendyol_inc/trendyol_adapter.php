<?php
	...
	class trendyol_adapter{
		...
		public function update_product_price_and_stock($barcode = '', $quantity = null, $discount_price = null, $sale_price = null){
			$trendyol_products = $this->trendyol->TrendyolMarketplaceProducts();
			$update_price      = $trendyol_products->update_product_price_and_stock($barcode, $quantity, $sale_price, $discount_price);

			return $update_price;
		}

		public function get_product_comments($sku = null, $page = 1, $ratings){
			$trendyol_question = $this->trendyol->TrendyolMarketplaceProducts();
			return $trendyol_question->get_product_comment($sku, ($page - 1), $ratings);
		}

		public function add_product_control($main_barcode = null, $barcode = null, $title = null, $description = null, $images = null, $vat_rate = null, $quantity = null, $discount_price = null, $sale_price = null, $brand_id = null, $category_id = null, $product_attributes = null, $cargo_company_id = null){
			...
			else if($discount_price > $sale_price){
			...
			if(is_null($category_id) or empty($category_id) or strlen($category_id) == 0){
				...
				if(!isset($get_cat_info->body->name)){
					$message         = 'Ürün Kategorisi Değeri Yanlıştır';
					$product_control = false;
					goto result;
				}
				else if(isset($get_cat_info->body->categoryAttributes[0])){
					foreach($get_cat_info->body->categoryAttributes as $attr){
		...
		public function update_product($barcode = null, $title = null, $description = null, $image = null, $vat_rate = 18, $quantity = 1, $discount_price = null, $sale_price = null, $brand_id = null, $category_id = null, $product_attributes = null){
			...
			$this->update_product_price_and_stock($barcode, $quantity, $discount_price, $sale_price);
		...
		public function add_product($main_code = null, $barcode = null, $title = null, $description = null, $images = null, $vat_rate = 20, $quantity = 1, $discount_price = null, $sale_price = null, $brand_id = null, $category_id = null, $product_attributes = null, $cargo_company_id = null){
			...
			return $trendyol_product->create_product([
				...
				'description'    => $description,
				'currencyType'   => 'TRY',
				'listPrice'      => str_replace(',', '.', $discount_price), //PSF - Satış fiyatı
		...
		public function add_multi_product($products = null){
			...
			foreach($products as $p_id => $product){
				...
				$discount_price   = $product['listPrice'] ?? null;
				$sale_price       = $product['salePrice'] ?? null;
				$attrs            = $product['attributes'] ?? null;
				$cargo_company_id = $product['cargoCompanyId'] ?? null;

				$product_control = $this->add_product_control($main_barcode, $barcode, $title, $desc, $images, $vat, $stock_qty, $discount_price, $sale_price, $brand_id, $trendyol_cat_id, $attrs, $cargo_company_id);
		...
		public function get_trendyol_to_website_suitable_product(){
			...
			if(isset($get_my_products)){
				...
				foreach($get_my_products->posts as $wc_product){
					...
					$product_discount_price       = get_post_meta($wc_product_id, 'wc_trendyol_product_trendyol_discount_price', true);
					...
					$product_control = $this->add_product_control($product_barcode, $product_title, $product_desc, $images, $product_vat, $product_stock_quantity, $product_discount_price, $product_sale_price, $product_brand[0], $trendyol_product_cat_id, $product_attributes, $cargo_company_id);
					if($product_control['status'] == 'success'){
						$my_products[$wc_product_id] = [
							...
							'currencyType'      => 'TRY',
							'listPrice'         => $product_discount_price,
		...
		public function trendyol_webhook_register($link){
			$trendyol_webhook = $this->trendyol->TrendyolMarketplaceWebhook();
			return $trendyol_webhook->create_webhook($link, ['CREATED', 'CANCELLED']);
		}
